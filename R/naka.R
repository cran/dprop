...
#' @import VaRES
#' @import stats
d_naka <- function(alpha, beta) {
pdf_E <- function(x,alpha, beta) {
    g=VaRES::dnakagami(x,alpha, beta)
    return(g)
  }

  naka_mom <- function(alpha, beta, r) {
  ...
  m1 <- naka_mom(alpha, beta, 1)
  m2 <- naka_mom(alpha, beta, 2)
  m3 <- naka_mom(alpha, beta, 3)
  m4 <- naka_mom(alpha, beta, 4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_naka <- VaRES::varnakagami(0.5, alpha, beta)

  qd_naka <- (VaRES::varnakagami(0.75, alpha, beta)-VaRES::varnakagami(0.25, alpha, beta))/2
