...
#' @import VaRES
#' @import stats
d_gompertz <- function(alpha, beta) {
pdf_E<-function(x, alpha, beta) {
    g=extraDistr::dgompertz(x, alpha, beta)
    return(g)
  }

  gom_mom <- function(alpha, beta, r) {
  ...
  m1 <- gom_mom(alpha, beta, 1)
  m2 <- gom_mom(alpha, beta, 2)
  m3 <- gom_mom(alpha, beta, 3)
  m4 <- gom_mom(alpha, beta, 4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_gompertz <- extraDistr::qgompertz(0.5, alpha, beta)

  qd_gompertz <- (extraDistr::qgompertz(0.75, alpha, beta)-extraDistr::qgompertz(0.25, alpha, beta))/2
  ...
  aux5 <- cbind(med_gompertz)
  colnames(aux5) = c("")

  aux6 <- cbind(qd_gompertz)
  colnames(aux6) = c("")
