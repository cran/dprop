#' @export
#' @import VaRES
d_nh <- function(alpha, beta) {
pdf_E <- function(x, alpha, beta) {
    g=VaRES::dexpext(x, alpha, beta)
    return(g)
  }

  nh_mom <- function(alpha, beta, r){
  ...
  m1 <- nh_mom(alpha, beta, 1)
  m2 <- nh_mom(alpha, beta, 2)
  m3 <- nh_mom(alpha, beta, 3)
  m4 <- nh_mom(alpha, beta, 4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_nh <- VaRES::varexpext(0.5, alpha, beta)

  qd_nh <- (VaRES::varexpext(0.75, alpha, beta)-VaRES::varexpext(0.25, alpha, beta))/2
