...
#' @import VaRES
#' @import stats
d_gumbel <- function(alpha, beta) {
pdf_E <- function(x,alpha, beta){
    g=extraDistr::dgumbel(x,alpha, beta)
    return(g)
  }

  gumbel_mom <- function(alpha, beta, r) {
  ...
  m1 <- gumbel_mom(alpha,beta,1)
  m2 <- gumbel_mom(alpha,beta,2)
  m3 <- gumbel_mom(alpha,beta,3)
  m4 <- gumbel_mom(alpha,beta,4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_gumbel <- extraDistr::qgumbel(0.5,alpha, beta)

  qd_gumbel <- (extraDistr::qgumbel(0.75,alpha, beta)-extraDistr::qgumbel(0.25,alpha, beta))/2
  ...
  aux5 <- cbind(med_gumbel)
  colnames(aux5) = c("")

  aux6 <- cbind(qd_gumbel)
  colnames(aux6) = c("")
