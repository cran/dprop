...
#' @import stats
#' @import VaRES
d_ingam <- function(alpha, beta) {
pdf_E <- function(x, alpha, beta) {
    g=extraDistr::dinvgamma(x, alpha, beta)
    return(g)
  }

  ingam_mom <- function(alpha, beta, r) {
  ...
  m1 <- ingam_mom(alpha, beta, 1)
  m2 <- ingam_mom(alpha, beta, 2)
  m3 <- ingam_mom(alpha, beta, 3)
  m4 <- ingam_mom(alpha, beta, 4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_invgamma <- extraDistr::qinvgamma(0.5, alpha, beta)

  qd_invgamma <- (extraDistr::qinvgamma(0.75, alpha, beta)-extraDistr::qinvgamma(0.25, alpha, beta))/2
  ...
  aux5 <- cbind(med_invgamma)
  colnames(aux5) = c("")

  aux6 <- cbind(qd_invgamma)
  colnames(aux6) = c("")
