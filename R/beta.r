#' @export
#' @import stats
d_beta <- function(alpha, beta) {
pdf_E <- function(x,alpha, beta) {
g=stats::dbeta(x,alpha, beta)
return(g)
}

beta_mom <- function(alpha, beta, r) {
f <- function(alpha, beta, r, x) {
(x^(r))*(pdf_E(x,alpha, beta))
}
y=integrate(f,lower=0,upper=1,subdivisions=1000000,alpha=alpha,beta=beta, r=r)$value
return(y)
}
  m1 <- beta_mom(alpha, beta, 1)
  m2 <- beta_mom(alpha, beta, 2)
  m3 <- beta_mom(alpha, beta, 3)
  m4 <- beta_mom(alpha, beta, 4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_beta <- stats::qbeta(0.5, alpha, beta)

  qd_beta <- (stats::qbeta(0.75, alpha, beta)-stats::qbeta(0.25, alpha, beta))/2
