...
d_EE <- function(alpha, beta) {
pdf_E <- function(x, alpha, beta) {
    g=VaRES::dexpexp(x, alpha, beta)
    return(g)
  }

  EE_mom <- function(alpha, beta, r) {
  ...
  m1 <- EE_mom(alpha, beta, 1)
  m2 <- EE_mom(alpha, beta, 2)
  m3 <- EE_mom(alpha, beta, 3)
  m4 <- EE_mom(alpha, beta, 4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_EE <- VaRES::varexpexp(0.5, alpha, beta)

  qd_EE <- (VaRES::varexpexp(0.75, alpha, beta)-VaRES::varexpexp(0.25, alpha, beta))/2
