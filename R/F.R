#' @export
#' @import VaRES
d_F <- function(alpha, beta) {
pdf_E <- function(x, alpha, beta) {
    g=VaRES::dF(x, alpha, beta)
    return(g)
  }

  F_mom <- function(alpha, beta, r) {
  ...
  m1 <- F_mom(alpha, beta, 1)
  m2 <- F_mom(alpha, beta, 2)
  m3 <- F_mom(alpha, beta, 3)
  m4 <- F_mom(alpha, beta, 4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_F <- VaRES::varF(0.5, alpha, beta)

  qd_F <- (VaRES::varF(0.75, alpha, beta)-VaRES::varF(0.25, alpha, beta))/2
