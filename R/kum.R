...
#' @import VaRES
#' @import stats
d_kum <- function(alpha, beta) {
pdf_E <- function(x, alpha, beta) {
    g=VaRES::dkum(x, alpha, beta)
    return(g)
  }

  kum_mom <- function(alpha, beta, r) {
  ...
  m1 <- kum_mom(alpha, beta, 1)
  m2 <- kum_mom(alpha, beta, 2)
  m3 <- kum_mom(alpha, beta, 3)
  m4 <- kum_mom(alpha, beta, 4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_kum <- VaRES::varkum(0.5, alpha, beta)

  qd_kum <- (VaRES::varkum(0.75, alpha, beta)-VaRES::varkum(0.25, alpha, beta))/2
