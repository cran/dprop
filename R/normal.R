#' @export
#' @import stats
d_normal <- function(alpha, beta) {
pdf_E <- function(x,alpha, beta){
g=stats::dnorm(x,alpha, beta)
return(g)
}

norm_mom<-function(alpha, beta, r) {
...
return(y)
  ...
  m1 <- norm_mom(alpha, beta, 1)
  m2 <- norm_mom(alpha, beta, 2)
  m3 <- norm_mom(alpha, beta, 3)
  m4 <- norm_mom(alpha, beta, 4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_normal <- stats::qnorm(0.5, alpha, beta)

  qd_normal <- (stats::qnorm(0.75, alpha, beta)-stats::qnorm(0.25, alpha, beta))/2
  ...
  aux5 <- cbind(med_normal)
  colnames(aux5) = c("")

  aux6 <- cbind(qd_normal)
  colnames(aux6) = c("")
