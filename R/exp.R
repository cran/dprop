#' @export
#' @import stats
d_exp <- function(alpha) {
pdf_E <- function(alpha, x) {
    g=alpha*exp(-alpha*x)
    return(g)
  }

  E_mom <- function(alpha, r) {
    f<-function(alpha, r, x) {
      (x^(r))*(pdf_E(alpha, x))
    }
    y=stats::integrate(f,lower=0,upper=Inf,subdivisions=1000000,alpha=alpha, r=r)$value
    return(y)
  }
  m1 <- E_mom(alpha, 1)
  m2 <- E_mom(alpha, 2)
  ...
  med_exp <- qexp(0.5, alpha)

  qd_exp <- (qexp(0.75, alpha)-qexp(0.25, alpha))/2
