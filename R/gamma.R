#' @export
#' @import stats
d_gamma <- function(alpha, beta) {
pdf_E <- function(x, alpha, beta) {
g=stats::dgamma(x, alpha, beta)
return(g)
}

gamma_mom <- function(alpha, beta, r) {
...
return(y)
  ...
  m1 <- gamma_mom(alpha, beta, 1)
  m2 <- gamma_mom(alpha, beta, 2)
  m3 <- gamma_mom(alpha, beta, 3)
  m4 <- gamma_mom(alpha, beta, 4)
  mu1 <- m1
  mu2 <- m2-(m1)^2
  ...
  med_gamma<-qgamma(0.5, alpha, beta)

  qd_gamma<-(qgamma(0.75, alpha, beta)-qgamma(0.25, alpha, beta))/2
